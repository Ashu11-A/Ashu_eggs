...
if [ -d "./php-fpm" ]; then # Caso alguem tenha o fpm já instalado, isso remove ele, pois esse projedo não precisa
    rm -rf ./php-fpm
fi
...
if [[ -d "./youtube-dl-web/frontend" ]]; then
    ...
    curl -sSL https://raw.githubusercontent.com/Ashu11-A/Ashu_eggs/main/Connect/pt-BR/Youtube-DL/start.sh -o start.sh
    chmod a+x ./start.sh
    bash <(curl -s https://raw.githubusercontent.com/Ashu11-A/Ashu_eggs/main/Connect/pt-BR/Youtube-DL/launch.sh)
else
    ...
    if [ ! -f "./logs/config_nginx" ]; then
        (
            ...
            rm default.conf
            wget https://raw.githubusercontent.com/Ashu11-A/Ashu_eggs/main/Connect/pt-BR/Youtube-DL/default.conf
